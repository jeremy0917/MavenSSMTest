...
import com.yjy.maven.utils.RedisDataSourceImpl;

@RunWith(SpringJUnit4ClassRunner.class)
// 表示继承了SpringJUnit4ClassRunner类
@ContextConfiguration(locations = { "classpath:spring-mybatis.xml" })
public class testRedisWithoutSpring {
	...
	public void test1() {		
		...
		try {
			...
			if (flag) {
				...
				logger.info(" userBillString : " + userBillString);
				return;
			}
		} catch (Exception e) {
			e.printStackTrace();
		...
		try {    
            redisClientTemplate.setex(key, 60, JSON.toJSONString(userBill));
        } catch (Exception e) {    
            e.printStackTrace();    
        }    
		logger.info("*************************");
